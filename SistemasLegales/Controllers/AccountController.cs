...
namespace SistemasLegales.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public AccountController(
            ...
            SistemasLegalesContext context)
        {
        ...
        public async Task<IActionResult> Users()
        {
            ...
            try
            {
                ...
                {
                    ApplicationUser UsuarioAutenticado = await _userManager.GetUserAsync(User);
                    lista= await _userManager.Users.Where(x=>x.IdEmpresa==UsuarioAutenticado.IdEmpresa).Include(x=>x.Empresa).Include(c => c.Roles).ToListAsync();
                }
                else 
                {
                    lista = await _userManager.Users.Include(x=>x.Empresa).Include(c => c.Roles).ToListAsync();
                }
        ...
        public async Task<IActionResult> Register(string id, string returnUrl = null)
        {
            try
            {
                ...
                if (User.IsInRole(Perfiles.AdministradorEmpresa))
                {
                    ViewData["Roles"] = new SelectList(new List<string>() { Perfiles.AdministradorEmpresa, Perfiles.Gerencia, Perfiles.Gestor });
                ...
                {
                    ViewData["Roles"] = new SelectList(new List<string>() { Perfiles.Administrador, Perfiles.AdministradorEmpresa, Perfiles.Gerencia, Perfiles.Gestor });
                ...
                if (!String.IsNullOrEmpty(id))
                {
                    ...
                    if (User.IsInRole(Perfiles.AdministradorEmpresa))
                        if (user.IdEmpresa != UsuarioAutenticado.IdEmpresa)
                            return this.Redireccionar($"{Mensaje.Error}|{Mensaje.UsuarioNoPerteneceEmpresa}", nameof(Users));
                    ...
                    return View(new RegisterViewModel
                    {
                        ...
                        Empresa=user.IdEmpresa.HasValue ? user.IdEmpresa.Value : 0,
                    });
                }
                return View();
            }
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    {
                        var user = new ApplicationUser {IdEmpresa=model.Empresa, UserName = model.UserName.ToLower(), Email = model.UserName.ToLower()};
        ...
        public async Task<IActionResult> Eliminar(string id)
        {
            try
            {
                ...
                if (user.IdEmpresa!=usuarioAutenticado.IdEmpresa)
                   return this.Redireccionar($"{Mensaje.Error}|{Mensaje.UsuarioNoPerteneceEmpresa}");
